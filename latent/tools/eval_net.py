...
parser.add_argument('dataset', type=str, choices=['ucf101', 'hmdb51', 'ixmas', 'ixmas_view', 'ixmas_branch', 'nucla', 'nucla_view'])
...
def eval_video(video):
    ...
    for tick in frame_ticks:
        if args.modality == 'rgb':
            ...
            view_all = net.predict_single_frame([frame,], 'arg_view', frame_size=(340, 256)).astype(int).flatten()
            view = np.bincount(view_all).argmax() # get the most within 10 variants
            scores = net.predict_single_frame([frame,], score_name+str(view+1), frame_size=(340, 256))
        ...
        if args.modality == 'flow':
            ...
            view_all = net.predict_single_flow_stack(flow_stack, 'arg_view', frame_size=(340, 256)).astype(int).flatten()
            view = np.bincount(view_all).argmax() # get the most within 10 variants
            scores = net.predict_single_flow_stack(flow_stack, score_name+str(view+1), frame_size=(340, 256))
            frame_scores.append(scores)
