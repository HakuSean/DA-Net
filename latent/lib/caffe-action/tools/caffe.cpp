...
void CopyLayers(caffe::Solver<float>* solver, const std::string& model_list) {
  ...
  boost::split(model_names, model_list, boost::is_any_of(",") ); // could finetune from more than one caffemodel file
  for (int i = 0; i < model_names.size(); ++i) {
    LOG(INFO) << "Finetuning from " << model_names[i]; // init caffemodel
