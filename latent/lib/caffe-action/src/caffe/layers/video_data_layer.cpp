...
void VideoDataLayer<Dtype>:: DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
	...
	if (this->layer_param_.video_data_param().shuffle()){
		...
		prefetch_rng_3_.reset(new Caffe::RNG(prefectch_rng_seed)); // for view
		ShuffleVideos();
	}
	...
	for (int i = 0; i < num_segments; ++i){
		...
		if (lines_duration_[lines_id_] < new_length + num_segments - 1){
			int offset = 0;
			offsets.push_back(offset);
		}
		else if (average_duration < new_length && lines_duration_[lines_id_] >= new_length + num_segments - 1){ // overlap should take place
			int offset = 0;
			offsets.push_back(offset+i*(average_duration-1));
		} 
		else {
			int offset = (*frame_rng)() % (average_duration - new_length + 1); // the minimum length should be 15, because the average_duration should be large than 5.
	...
	if (this->layer_param_.video_data_param().modality() == VideoDataParameter_Modality_FLOW)
		CHECK(ReadSegmentFlowToDatum(lines_[lines_id_].first, lines_[lines_id_].second, lines_view_[lines_id_], offsets, new_height, new_width, new_length, &datum, name_pattern_.c_str()));
	else
		CHECK(ReadSegmentRGBToDatum(lines_[lines_id_].first, lines_[lines_id_].second, lines_view_[lines_id_], offsets, new_height, new_width, new_length, &datum, is_color, name_pattern_.c_str()));
	...
	top[1]->Reshape(batch_size, 1, 1, 1); // action label
	top[2]->Reshape(batch_size, 1, 1, 1); // view label
	this->prefetch_label_.Reshape(batch_size, 1, 1, 1);
	this->prefetch_view_.Reshape(batch_size, 1, 1, 1);
...
void VideoDataLayer<Dtype>::ShuffleVideos(){
	...
	shuffle(lines_view_.begin(), lines_view_.end(),prefetch_rng3);
}
...
void VideoDataLayer<Dtype>::InternalThreadEntry(){
	...
	Dtype* top_view = this->prefetch_view_.mutable_cpu_data();
	...
	for (int item_id = 0; item_id < batch_size; ++item_id){
		...
		if (this->layer_param_.video_data_param().modality() == VideoDataParameter_Modality_FLOW){
			if(!ReadSegmentFlowToDatum(lines_[lines_id_].first, lines_[lines_id_].second, lines_view_[lines_id_], offsets, new_height, new_width, new_length, &datum, name_pattern_.c_str())) {
				continue;
			}
		} else{
			if(!ReadSegmentRGBToDatum(lines_[lines_id_].first, lines_[lines_id_].second, lines_view_[lines_id_], offsets, new_height, new_width, new_length, &datum, is_color, name_pattern_.c_str())) {
				continue;
			}
		}
		...
		top_view[item_id] = lines_view_[lines_id_]; // this output is correct
		//LOG(INFO)<< "top_label: " << top_label[item_id];
		//LOG(INFO)<< "top_view: " << top_view[item_id];
