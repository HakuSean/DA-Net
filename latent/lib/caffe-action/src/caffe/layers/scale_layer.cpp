...
void ScaleLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  outer_dim_ = bottom[0]->count(0, axis_); //0:1, 0~end:1680, 1~end:70, 2~end: 14, so 24,5,14,1 
  scale_dim_ = scale->count(); // for rgb with batch=24, 120 = 24*5, total number
  inner_dim_ = bottom[0]->count(axis_ + scale->num_axes()); // 1
...
void ScaleLayer<Dtype>::Forward_cpu(
  ...
  for (int n = 0; n < outer_dim_; ++n) {
    for (int d = 0; d < scale_dim_; ++d) {
      ...
      bottom_data += inner_dim_; // seems to move the pointer
      top_data += inner_dim_;
    }
  }
  if (bias_layer_) {
