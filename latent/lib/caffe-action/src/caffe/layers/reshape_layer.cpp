...
void ReshapeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  constant_count_ = 1;
  for (int i = 0; i < top_num_axes; ++i) { // 4 loops
    const int top_dim = top_blob_shape.dim(i); // each dim's value
    if (top_dim == 0) {
      ...
      constant_count_ *= top_dim; // the final value should be all the values in the blob
    }
  }
}
...
void ReshapeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const int input_start_axis = this->layer_param_.reshape_param().axis(); // default 0
  const int start_axis = (input_start_axis >= 0) ? input_start_axis :
      bottom[0]->num_axes() + input_start_axis + 1; // should be 0
  ...
  const int num_axes = this->layer_param_.reshape_param().num_axes(); // default -1
  CHECK_GE(num_axes, -1) << "num_axes must be >= 0, or -1 for all";
  const int end_axis =
      (num_axes == -1) ? bottom[0]->num_axes() : (start_axis + num_axes); // should be 4 
  CHECK_LE(end_axis, bottom[0]->num_axes())
      << "end_axis = axis + num_axes is out of range";
  const int num_axes_replaced = end_axis - start_axis; // default: 4-0 = 4
  const int num_axes_retained = bottom[0]->num_axes() - num_axes_replaced; // default: 4-4=0, nothing retained
  const BlobShape& top_blob_shape = this->layer_param_.reshape_param().shape(); // the dimensions
  const int num_new_axes = top_blob_shape.dim_size(); // should be 4
  vector<int> top_shape(num_axes_retained + num_new_axes); // 4, the size of the output, merge the retained axes and the new axes, this vector stores the new dimensions
  int top_shape_index = 0; 
  for (int i = 0; i < start_axis; ++i) { // start = 0
    top_shape[top_shape_index++] = bottom[0]->shape(i); 
  }
  for (int i = 0; i < num_new_axes; ++i) { // new = 4
    top_shape[top_shape_index++] = top_blob_shape.dim(i);
  }
  for (int i = end_axis; i < bottom[0]->num_axes(); ++i) {
    top_shape[top_shape_index++] = bottom[0]->shape(i);
  } // finish defining the sizes of each dim in the output
  CHECK_EQ(top_shape_index, top_shape.size());
  for (int i = 0; i < copy_axes_.size(); ++i) { // copy_axes: the axes that are copied from input
    const int copy_axis_index = copy_axes_[i]; // 0,1,2,or 3
