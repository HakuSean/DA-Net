...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  for (set<string>::iterator it = available_blobs.begin();
    ...
    LOG(INFO) << "This network produces output " << *it; // loss and accuracy
...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    Dtype layer_loss = layers_[i]->Forward(bottom_vecs_[i], top_vecs_[i]); // net.forward consists of all layers' forward.
    loss += layer_loss; // add all layer_loss together
...
const vector<Blob<Dtype>*>& Net<Dtype>::ForwardPrefilled(Dtype* loss) {
  ...
  // LOG(INFO) << "loss " << loss[0]; //output some number, which is the loss of each iteration
  // LOG(INFO) << "net_output_blobs_ " << net_output_blobs_.size() << " with value "<< net_output_blobs_[0]; // output 5 with value 0xc05d540 in training, size=10 when testing because there are five more accuracy outputs.
...
const vector<Blob<Dtype>*>& Net<Dtype>::Forward(
  ...
  for (int i = 0; i < bottom.size(); ++i) { // bottom.size()==0
...
string Net<Dtype>::Forward(const string& input_blob_protos, Dtype* loss) {
  ...
  for (int i = 0; i < net_output_blobs_.size(); ++i) { // 5 or 10 values, train is 5 and test is 10
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  // initial a bool to store whether the layer has been initialized
  vector <bool> Layer_initialized;
  Layer_initialized.resize(layer_names_.size()); // layer names are names in prototxt
  for (int i = 0; i < layer_names_.size(); ++i) {
    Layer_initialized[i] = false;
  }
  LOG(INFO) << "Finished initializing Layer_initialized";
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    const LayerParameter& source_layer = param.layer(i);
    const string& source_layer_name = source_layer.name();
    const string& source_layer_name_reg = source_layer_name + "/r2";
    int target_layer_id = 0;
    while (target_layer_id != layer_names_.size() &&
        layer_names_[target_layer_id] != source_layer_name_reg) {
      ++target_layer_id;
    }
    if (target_layer_id == layer_names_.size() || Layer_initialized[target_layer_id] ) {
      DLOG(INFO) << "Ignoring source layer " << source_layer_name_reg;
      continue;
    }
    DLOG(INFO) << "Copying source layer " << source_layer_name_reg;
    vector<shared_ptr<Blob<Dtype> > >& target_blobs =
        layers_[target_layer_id]->blobs();
    CHECK_EQ(target_blobs.size(), source_layer.blobs_size())
        << "Incompatible number of blobs for layer " << source_layer_name_reg;
    for (int j = 0; j < target_blobs.size(); ++j) {
      const bool kReshape = false;
      target_blobs[j]->FromProto(source_layer.blobs(j), kReshape);
    }
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    const LayerParameter& source_layer = param.layer(i);
    const string& source_layer_name = source_layer.name();
    const string& source_layer_name_reg = source_layer_name + "/r3";
    int target_layer_id = 0;
    while (target_layer_id != layer_names_.size() &&
        layer_names_[target_layer_id] != source_layer_name_reg) {
      ++target_layer_id;
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    const LayerParameter& source_layer = param.layer(i);
    const string& source_layer_name = source_layer.name();
    const string& source_layer_name_reg = source_layer_name + "/r4";
    int target_layer_id = 0;
    while (target_layer_id != layer_names_.size() &&
        layer_names_[target_layer_id] != source_layer_name_reg) {
      ++target_layer_id;
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    const LayerParameter& source_layer = param.layer(i);
    const string& source_layer_name = source_layer.name();
    const string& source_layer_name_reg = source_layer_name + "/r5";
    int target_layer_id = 0;
    while (target_layer_id != layer_names_.size() &&
        layer_names_[target_layer_id] != source_layer_name_reg) {
      ++target_layer_id;
