...
void Solver<Dtype>::Test(const int test_net_id) {
  ...
  Dtype loss = 0;
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) { // each test_iter will generate one iter_loss, the final loss is the summation of the iter_loss
    Dtype iter_loss;
    const vector<Blob<Dtype>*>& result =
        test_net->Forward(bottom_vec, &iter_loss); // this is where the loss has been calculated
#ifdef USE_MPI
    if (Caffe::parallel_mode() == Caffe::MPI) {
      SyncOutput(test_net);
    ...
    if (i == 0) { 
      for (int j = 0; j < result.size(); ++j) { //result: some are loss, some are acc; 1+1, or 5+5
        const Dtype* result_vec = result[j]->cpu_data();
        for (int k = 0; k < result[j]->count(); ++k) {// suppose k has length of 10
          test_score.push_back(result_vec[k]);
          test_score_output_id.push_back(j); // initialize the test_score, test_score_output_id.
        }
      }
    } else {
      int idx = 0;
      for (int j = 0; j < result.size(); ++j) { // result has the length of loss+acc, e.g. 10.
        const Dtype* result_vec = result[j]->cpu_data();
        for (int k = 0; k < result[j]->count(); ++k) { // k only has one number.
          test_score[idx++] += result_vec[k];
        }
      }
    }// there are 2 or 10 scores in total, but each instance will have a 10-dim vector, and they should be combined accordingly. That's why when i==0, give initialization of test_score, and in other cases, just add together.
  }
  if (param_.test_compute_loss()) { // default as false
    loss /= param_.test_iter(test_net_id);
    LOG(INFO) << "Test loss: " << loss; //this loss is just the same as test_scores. If branch number goes up, then the loss is summation of all branch losses.
  }

  for (int i = 0; i < test_score.size(); ++i) { // five loss, five acc
    const int output_blob_index =
        test_net->output_blob_indices()[test_score_output_id[i]];
    const string& output_name = test_net->blob_names()[output_blob_index];
    const Dtype loss_weight = test_net->blob_loss_weights()[output_blob_index];
    ostringstream loss_msg_stream;
    Dtype mean_score = test_score[i]/ param_.test_iter(test_net_id); // mean score and mean loss, param_.test_iter(test_net_id) cannot be zeros because they are predefined.
