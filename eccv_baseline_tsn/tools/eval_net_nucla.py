...
parser.add_argument('dataset', type=str, choices=['ucf101', 'hmdb51', 'ixmas', 'ixmas_view', 'ixmas_branch', 'ixmas_gender', 'nucla', 'act42', 'nucla_view'])
...
parser.add_argument('--num_frame_per_video', type=int, default=5,
...
def eval_video(video):
    ...
    label = video[1]
    vid = video[0]
    view = eval(video[2]) #+1 for ixmas

    if view == 1:
        score_name_view = score_name
    else:
        score_name_view = score_name+'_reg'+str(view)
    ...
    for tick in frame_ticks:
        if args.modality == 'rgb':
            ...
            scores = net.predict_single_frame([frame,], score_name_view, frame_size=(340, 256))
        ...
        if args.modality == 'flow':
            ...
            scores = net.predict_single_flow_stack(flow_stack, score_name_view, frame_size=(340, 256))
            frame_scores.append(scores)
