...
def parse_log(path_to_log):
    ...
    regex_semantic_row = re.compile('.cpp:(\d+)')
    ...
    with open(path_to_log) as f:
        ...
        for line in f:
            semantic_match = regex_semantic_row.search(line)
            if not semantic_match:
                # My logs has some end errors like "Setting device 1"
                continue
...
def main():
    ...
    output_dir = os.path.dirname(args.logfile_path)#+'_parse'
